...
class BT_InvertBinaryTreeTest {
    ...
    private boolean compareBT(BT_InvertBinaryTree.BinaryTree tree1, InvertedBinaryTree tree2) {
    ...
    class TestBinaryTree extends BT_InvertBinaryTree.BinaryTree {
        ...
        public void insert(int[] values, int i) {
            ...
            ArrayDeque<BT_InvertBinaryTree.BinaryTree> queue = new ArrayDeque<BT_InvertBinaryTree.BinaryTree>();
            queue.addLast(this);
            while (queue.size() > 0) {
                BT_InvertBinaryTree.BinaryTree current = queue.pollFirst();
                if (current.left == null) {
                    current.left = new BT_InvertBinaryTree.BinaryTree(values[i]);
                    break;
                }
                queue.addLast(current.left);
                if (current.right == null) {
                    current.right = new BT_InvertBinaryTree.BinaryTree(values[i]);
                    break;
                }
                queue.addLast(current.right);
            }
